...
- 也可以把这个事件一个队列里，之后再调度这个队列去处理io读写，调度的过程可以仍然由这个线程完成也可以由其他线程完成，比如java的Reactor，go语言的协程实现。第二种设计中，会使用一个线程(go:网络轮询器?)监听select或epoll等待系统io事件，而用和**cpu核心数相同的线程数**(go的MPG)处理io读写事件。
